...
<body class="bg-gray-50">
    ...
    <header class="bg-white shadow-sm border-b">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="flex justify-between items-center py-4">
                ...
                <div class="flex items-center space-x-4">
                    <a href="/admin" class="bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-lg transition-colors">
                        <i class="fas fa-cog mr-2"></i>Login JARTI
                    </a>
                </div>
            </div>
        </div>
    ...
    <section class="bg-blue-600 text-white py-16">
        <div class="max-w-4xl mx-auto px-4 text-center">
            ...
            <p class="text-blue-100 mb-8 text-lg">Temukan jawaban dari pertanyaan yang sering ditanyakan</p>
        </div>
    </section>
    ...
    <main class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
                ...
                <div class="bg-white rounded-lg shadow-md hover:shadow-lg transition-shadow cursor-pointer border category-card" 
                    ...
                    <div class="p-6">
                        <div class="flex items-center mb-4">
                            ...
                            <div>
                                <h4 class="font-semibold text-gray-900">{{ $category->kbc_name }}</h4>
                                <p class="text-sm text-gray-600">{{ $category->knowledgebase->count() }} artikel</p>
                            </div>
                        </div>
        ...
        <section class="mb-16">
            <div class="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4">
                <h3 class="text-2xl font-bold text-gray-900">Semua Artikel</h3>
                <div class="flex flex-wrap gap-2">
                    <button class="filter-btn active px-4 py-2 rounded-lg bg-blue-600 text-white" data-filter="all">
                        Semua
                    </button>
                    @foreach($categories as $category)
                    <button class="filter-btn px-4 py-2 rounded-lg bg-gray-200 text-gray-700 hover:bg-gray-300 transition-colors" 
                            data-filter="{{ $category->kbc_id }}">
                        {{ $category->kbc_name }}
                    </button>
                    @endforeach
                </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6" id="articlesGrid">
                ...
                <article class="bg-white rounded-lg shadow-md hover:shadow-lg transition-shadow kb-article" 
                         data-category="{{ $kb->kbc_id ?? 'uncategorized' }}">
                    <div class="p-6">
                        <div class="flex items-center mb-3">
                            @if($kb->category)
                            <span class="inline-block bg-blue-100 text-blue-800 text-xs px-2 py-1 rounded-full mr-2">
                                {{ $kb->category->kbc_name }}
                            </span>
                            @else
                        ...
                        <h4 class="font-semibold text-gray-900 mb-3 line-clamp-2">{{ $kb->kb_name }}</h4>
                        
                        @if($kb->kb_content)
                        <p class="text-gray-600 text-sm mb-4 line-clamp-3">{{ Str::limit(strip_tags($kb->kb_content), 120) }}</p>
                        @endif
                        
                        <div class="flex items-center justify-between">
                            <button class="text-blue-600 hover:text-blue-700 font-medium text-sm flex items-center"
                                    onclick="openModal('{{ $kb->kb_id }}')">
    ...
    <footer class="bg-gray-800 text-white py-12">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="grid grid-cols-1 md:grid-cols-3 gap-8">
                <div>
                    <h5 class="font-semibold mb-4">Knowledge Base</h5>
                    <p class="text-gray-300 text-sm">
                        Pusat informasi dan panduan untuk membantu Anda menemukan solusi dengan cepat.
                    </p>
                </div>
                <div>
                    ...
                    <ul class="space-y-2 text-sm text-gray-300">
                        ...
                        <li>
                            <a href="#" class="hover:text-white transition-colors">{{ $category->kbc_name }}</a>
                        </li>
                        @endforeach
                    </ul>
                </div>
    ...
    <div id="articleModal" class="fixed inset-0 bg-black bg-opacity-50 z-50 hidden">
    ...
    <script>
        ...
        categoryCards.forEach(card => {
            card.addEventListener('click', function() {
                ...
                const filterBtn = document.querySelector(`[data-filter="${categoryId}"]`);
                if (filterBtn) {
                    filterBtn.click();
                }
            });
        ...
        // Modal functionality
        const knowledgeData = @json($allKnowledge->keyBy('kb_id'));
        
        function openModal(kbId) {
            const kb = knowledgeData[kbId];
            if (kb) {
                document.getElementById('modalTitle').textContent = kb.kb_name;
                document.getElementById('modalContent').innerHTML = kb.kb_content || 'Tidak ada konten tersedia.';
